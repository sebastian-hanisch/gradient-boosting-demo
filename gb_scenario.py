...
def add_outliers(y_reg_train, outlier_pct, seed):
    """Grobe Ausreißer NUR im Training (Regression): ein zufälliger Anteil der Zeilen bekommt einen großen additiven Schock (60 bis 120 Minuten, zufälliges Vorzeichen) - grobe Messfehler oder
    Sonderfahrten, die nichts mit den Merkmalen zu tun haben. Der Test bleibt unverändert (misst also gegen die wahre Dauer)."""
    if outlier_pct <= 0:
        return y_reg_train
    rng = np.random.default_rng([int(seed), 555])
    n = len(y_reg_train)
    mask = rng.random(n) < float(outlier_pct) / 100.0
    shock = rng.choice([-1.0, 1.0], n) * rng.uniform(60.0, 120.0, n)
    y2 = y_reg_train.copy()
    y2[mask] += shock[mask]
    return y2
