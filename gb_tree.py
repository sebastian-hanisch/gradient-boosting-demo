"""Der CART-Baumkern (aus cart-demo übernommen, dort ausführlich geprüft) - Gradient Boosting braucht davon nur die REGRESSIONSBÄUME (Varianz-Kriterium): jede Boosting-Runde wächst einen Regressionsbaum auf den
...
def set_leaf_values(tree, X, compute):
    ...
    for t in np.nonzero(tree.feature < 0)[0]:
        idx = np.nonzero(leaf == t)[0]
        if len(idx):
            new_value[t] = compute(idx)
    return replace(tree, value=new_value)
